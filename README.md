...
2. From `/containers/` folder you can delete all other folder except `App` and `Error`
3. From `/assets/image/` folder you can delete all other folder except `app`. Do not delete any files from there like `404.svg`, `error.svg` etc.
4. From `/data/` folder you can delete all other folder except `App`.
5. From `/theme/` folder you can delete all other folder except `app`. Do not delete the `index.js` file.
...
```
yarn next-export
docker build -t superprops/landing-next -f ./Dockerfile .
...
go to `/package.json`, find engines and do the following
